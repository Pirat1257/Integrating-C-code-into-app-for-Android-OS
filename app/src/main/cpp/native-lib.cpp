...
Java_com_example_bos_14_File_1work_AES(JNIEnv *env, jobject thiz,
        jstring _path, // Путь до файла
        jint _what, // 0 - шифрование, 1 - расшифрование
        jint size // Размер исходного файла
        ) {
    char *path = const_cast<char *>(env->GetStringUTFChars(_path, 0));
    ...
    unsigned char key[] = {0x01, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a,
                           0x0b, 0x0c, 0x0d, 0x0e, 0x0f}; //key example
    unsigned int outLen = 0;  // out param - bytes in сiphertext

    AES aes(128);  //128 - key length, can be 128, 192 or 256
    // Шифрование
    if (_what == 0) {
        char *c = reinterpret_cast<char *>(aes.EncryptECB(reinterpret_cast<unsigned char *>(buffer),
                                                          file_size, key, outLen));
        free(buffer);
        FILE *output;
        output = fopen(path, "w+");
        if (output == NULL) {
    ...
    // Расшифрование
        ...
        char *c = reinterpret_cast<char *>(aes.DecryptECB(reinterpret_cast<unsigned char *>(buffer),
                                                          file_size, key));
        free(buffer);
        FILE *output;
        output = fopen(path, "w+");
